...
public class FileAttribute{
    ...
    public FileAttribute(SFile sFile) {
        ...
        JLabel labelIcon = new JLabel(new ImageIcon(new ImageIcon(getClass().getResource("/Icon/sfileAttribute.jpg")).getImage().getScaledInstance(25, 30, Image.SCALE_SMOOTH)));
        // JLabel labelIcon=new JLabel("image/folder2.png");
        textField = new JTextField(sFile.getName());
        textField.setEditable(false);
                
        labelType = new JLabel(" 类型   :   Folder ");
        labelPath = new JLabel(" 路径   :");
        labelGetPath = new JLabel(CurrentPath.getString(sFile.getPath()));
        labelSpace = new JLabel("占用空间大小：");
        labelGetSpace = new JLabel(String.format("%d", sFile.getLength()));
        ...
        panelType.setBounds(26, 80, 100, 30);
        ...
        checkBox1.setBounds(90, 260, 60, 30);
        panelAll.add(checkBox2);
        checkBox2.setBounds(160, 260, 60, 30);
        if(sFile.isReadonly()) checkBox1.setSelected(true);
        else checkBox1.setSelected(false);
        if(sFile.isHidden()) checkBox2.setSelected(true);
        ...
        panelButton.setBounds(50, 310, 220, 30);*/
        ...
        bTrue.addActionListener((ActionEvent e) -> {
            if(checkBox1.isSelected()) sFile.setReadonly(true);
            else sFile.setReadonly(false);
            if(checkBox2.isSelected()) sFile.setHidden(true);
            else sFile.setHidden(false);
            sFile.getParent().sonChanged(sFile);//SFile类型肯定有parent
            frame.dispose();
        });
